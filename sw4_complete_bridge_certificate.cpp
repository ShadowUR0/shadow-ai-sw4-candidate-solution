...
int main() {
    constexpr i64 OLD_COVERAGE_END = 292031545;
    constexpr i64 INFINITE_TAIL_START = 275233782943;
    ...
    std::ofstream table("sw4_complete_bridge_output.txt");
    assert(table);
    ...
    // The first of these intervals begins below OLD_COVERAGE_END.
    run_chain("seed51", 63, 66, seed51, 63, {});
    run_chain("seed65", 67, 206, seed65, 2047, {});

    // The annuli occupy 101..140 and 141..204. They are valid for h>=206.
    run_chain("seed100_TM", 206, 253, seed100, 262143, {{101,5},{141,8}});

    assert(first_new_start == 282204658);
    assert(covered_end == 554860689583);
    assert(covered_end >= INFINITE_TAIL_START - 1);
    ...
    std::cout << "infinite_tail_start=" << INFINITE_TAIL_START << '\n';
    std::cout << "continuous_coverage=6572800..infinity (using supplied earlier certificates)\n";
    return 0;
}
